...
>
  <div
    class="sm:w-[355px] w-full sm:h-[200px] flex justify-center items-center relative"
  >
    <div
        ...
        <Drawer.Content class="px-2">
          ...
          <Drawer.Footer>
            <Button
              on:click={() => {
                addToCart({
                  ...
                  size: {
                    ...
                    stripePriceId:
                      itemData.sizes[selectedSizeIdx].stripePriceId,
              ...
              >Add to cart ${(
                itemData.sizes[selectedSizeIdx].price / 100
              ).toFixed()}</Button
            >
      ...
      {#each itemData.sizes as size, i}
        ...
        {:else}
          <span
            class={`text-xs ${
              size.isAvailable ? "bg-white" : "line-through opacity-55"
            }  rounded-full px-2 py-1`}>{size.width} x {size.height}</span
          >
        {/if}
      {/each}
    </div>
  ...
  <div
    class="sm:flex flex-col hidden gap-y-2 items-center justify-center sm:w-[175px] w-full"
  >
    <Button
